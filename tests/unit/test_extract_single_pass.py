"""Unit tests for the single-pass DB-backed extract agent."""
...
def test_build_extract_agent_wires_db_tool_surface(tmp_path) -> None:
	"""The extract agent should expose the six DB-era extract tools."""
...
def test_extract_tools_take_contextdeps_runcontext() -> None:
	...
	for fn in (trace_read, context_search, context_fetch, context_apply, note, prune):
...
def test_system_prompt_mentions_db_only_flow() -> None:
	"""The extract prompt should teach the DB-only episode-plus-durable-record flow."""
	assert "episode" in SYSTEM_PROMPT.lower()
	assert "durable records" in SYSTEM_PROMPT.lower()
	assert "context_apply" in SYSTEM_PROMPT
	assert "Do not talk about filenames" in SYSTEM_PROMPT
	assert "decision" in SYSTEM_PROMPT.lower()


def test_run_extraction_signature_matches_db_inputs() -> None:
	"""The extract runner now takes DB path, project identity, and session ID."""
