...
def test_curate_routine_episode_archived(
    ...
    episode = next(
        ...
        if record["record_id"] == "rec_routine_episode"
    )
    ...
    latest_change_kinds = {
        str(version["change_kind"]) for version in episode["versions"][:2]
    }
...
def test_curate_verbose_episode_compressed(
    ...
    episode = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_verbose_episode"
    )
    assert outcome.result.completion_summary.strip()
    ...
    assert (
        episode["user_intent"]
        != "Review the storage-boundary migration and decide whether the split still makes sense."
    )
    assert (
        episode["what_happened"]
        != "Compared the queue path, product context path, migration plan, and temporary implementation concerns in one long review."
    )
    assert (
        episode["outcomes"]
        != "Ended with the same storage-boundary decision but kept too much session story."
    )
    sentence_count = len(
        [part for part in re.split(r"[.!?]+", str(episode["body"])) if part.strip()]
    )
    ...
    topic_hits = sum(
        1
        for token in expectation["episode_text_must_include_any"]
        if token in episode_text
    )
    assert topic_hits >= 2
...
def test_curate_durable_record_rewritten_from_session_report_style(
    ...
    expectation = load_curate_expectation(
        "durable_record_rewritten_from_session_report_style"
    )["expected"]
    outcome = run_curate_case(
    ...
    record = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_provider_review"
    )
    assert outcome.result.completion_summary.strip()
    assert record["status"] == "active"
    latest_change_kinds = {
        str(version["change_kind"]) for version in record["versions"][:2]
    }
    assert "update" in latest_change_kinds

    record_text = _normalize_assertion_text(
        " ".join(
...
def test_curate_valuable_recent_record_preserved(
    ...
    expectation = load_curate_expectation("valuable_recent_record_preserved")[
        "expected"
    ]
    outcome = run_curate_case(
    ...
    record = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_recent_provider_record"
    )
    assert outcome.result.completion_summary.strip()
    assert record["status"] == "active"
...
def test_curate_mixed_store_cleanup(
    ...
    routine_episode = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_mixed_routine_episode"
    )
    weak_duplicate = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_mixed_duplicate_weak"
    )
    strong_duplicate = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_mixed_duplicate_strong"
    )
    rewritten = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_mixed_session_report"
    )
    ...
    rewritten_text = _normalize_assertion_text(
        " ".join(
    ...
    for row in outcome.changed_version_rows:
        per_record_kinds.setdefault(str(row["record_id"]), set()).add(
            str(row["change_kind"])
        )
...
def test_curate_meaningful_episode_preserved_with_durable_neighbor(
    ...
    expectation = load_curate_expectation(
        "meaningful_episode_preserved_with_durable_neighbor"
    )["expected"]
    outcome = run_curate_case(
    ...
    episode = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_meaningful_episode"
    )
    assert episode["status"] == "active"
    assert episode["superseded_by_record_id"] in (None, "")
    if episode["record_id"] in {
        str(row["record_id"]) for row in outcome.changed_version_rows
    }:
        kinds = {
            str(row["change_kind"])
            for row in outcome.changed_version_rows
            if str(row["record_id"]) == episode["record_id"]
        }
        assert kinds == {"update"}
...
def test_curate_concise_report_style_durable_rewritten(
    ...
    expectation = load_curate_expectation("concise_report_style_durable_rewritten")[
        "expected"
    ]
    outcome = run_curate_case(
    ...
    record = next(
        record
        for record in outcome.records
        if record["record_id"] == "rec_concise_report_decision"
    )
    assert outcome.result.completion_summary.strip()
