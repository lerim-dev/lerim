...
def _bar(width_pct: float, *, x: int, y: int, max_width: int) -> str:
...
def build_svg(snapshot: BenchmarkSnapshot) -> str:
    ...
    return "\n".join(
        [
            ...
            f'  <text x="78" y="132" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#53645b">{html.escape(subtitle)}</text>',
            "",
            ...
            f'    <text x="26" y="74" font-family="Arial, Helvetica, sans-serif" font-size="17" fill="#53645b">Hybrid search over {snapshot.question_count} questions.</text>',
            f"    {_bar(snapshot.recall_at_5, x=26, y=104, max_width=320)}",
            f'    <text x="366" y="124" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="700" fill="#17382a">R@5 {recall_5}</text>',
            f"    {_bar(snapshot.recall_at_10, x=26, y=148, max_width=320)}",
            f'    <text x="366" y="168" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="700" fill="#17382a">R@10 {recall_10}</text>',
            f"    {_bar(snapshot.recall_at_20, x=26, y=192, max_width=320)}",
            f'    <text x="366" y="212" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="700" fill="#17382a">R@20 {recall_20}</text>',
            ...
            f"    {_bar(snapshot.context_reduction, x=26, y=112, max_width=330)}",
            f'    <text x="378" y="135" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="700" fill="#17382a">{reduction}</text>',
            f'    <text x="26" y="184" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#53645b">Token reduction with {context_recall} recall.</text>',
            ...
            f'    <text x="24" y="82" font-family="Arial, Helvetica, sans-serif" font-size="20" fill="#53645b">{snapshot.latency_record_count} records, local search path</text>',
            f'    <text x="24" y="124" font-family="Arial, Helvetica, sans-serif" font-size="27" font-weight="700" fill="#17382a">p50 {p50}</text>',
            f'    <text x="216" y="124" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#53645b">p99 {p99}</text>',
            ...
            f'    <text x="24" y="88" font-family="Arial, Helvetica, sans-serif" font-size="31" font-weight="700" fill="#17382a">{snapshot.mcp_config_passed}/{snapshot.mcp_config_total}</text>',
            '    <text x="128" y="88" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#53645b">config probes passed.</text>',
            f'    <text x="344" y="88" font-family="Arial, Helvetica, sans-serif" font-size="18" fill="#53645b">Gemini live tool-call: {snapshot.gemini_live_tool_calls}</text>',
            f'    <text x="24" y="124" font-family="Arial, Helvetica, sans-serif" font-size="17" fill="#53645b">Synthetic trace-submit extraction: {snapshot.trace_submit_extraction_acceptances}</text>',
