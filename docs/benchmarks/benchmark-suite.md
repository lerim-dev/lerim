...
| MCP integration | Config writer coverage, stdio MCP probes, synthetic trace-submit protocol acceptance, and selected installed-client acceptances | Completed-session capture for every client or organic production traces | `benchmarks/results/raw/mcp-integration-full/report.json` and `benchmarks/results/raw/mcp-gemini-live-tool-call/report.json` |
